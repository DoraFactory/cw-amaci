use cosmwasm_std::{coins, from_json, Uint128, Uint256};
...
fn create_round_with_reward_should_works() {
    let user1_coin_amount = 1000000000000000000000u128; // 1000 DORA (register 500, create round 50)
    let operator_coin_amount = 1000000000000000000000u128; // 1000 DORA
    ...
    let base_operator_stake_amount = 500000000000000000000u128;
    _ = contract.set_maci_operator(
        &mut app,
        user1(),
        operator(),
        &coins(base_operator_stake_amount, DORA_DEMON),
    );
    let user1_operator_addr = contract.get_validator_operator(&app, user1()).unwrap();
    assert_eq!(operator(), user1_operator_addr);
    ...
    let amaci_round_balance = contract.balance_of(
        &app,
    ...
    let total_fee = Uint128::from(small_base_payamount + small_base_payamount);
    let admin_fee = circuit_charge_config.fee_rate * total_fee;
    let operator_fee = total_fee - admin_fee;

    assert_eq!(
        Uint128::from(operator_fee),
